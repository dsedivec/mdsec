 ...
 */
export function resolveVersion(packageRoot) {
    if (existsSync(join(packageRoot, ".git"))) {
        try {
            return execFileSync("git", ["describe", "--tags", "--always", "--dirty"], { cwd: packageRoot, encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }).trim();
        }
        catch {
            // fall through
        }
    ...
    try {
        const stamp = JSON.parse(readFileSync(join(packageRoot, "dist", "version-stamp.json"), "utf8"));
