...
export function unifiedDiff(oldText, newText, label, color) {
    if (oldText === newText)
        return "";
    ...
    for (const c of changed) {
        ...
        if (last && c - last.to <= 2 * CONTEXT + 1)
            last.to = c;
        else
            groups.push({ from: c, to: c });
    }
    const paint = (code, line) => color ? `${code}${line}${RESET}` : line;
    const out = [
        paint(RED, `--- ${label}`),
    ...
    for (const g of groups) {
        ...
        out.push(paint(CYAN, `@@ -${oldLineAt[from]},${oldLen} +${newLineAt[from]},${newLen} @@`));
        for (let k = from; k <= to; k++) {
            const op = ops[k];
            ...
            if (op.tag === "-")
                out.push(paint(RED, line));
            else if (op.tag === "+")
                out.push(paint(GREEN, line));
            else
                out.push(line);
            if (noEol)
